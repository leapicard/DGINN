	...
	with open(recTree, "r") as tree:
		...
		if len(lOut) > 0:
			...
			if len(dRemain.keys()) > int(nbSp) - 1:
				...
				logger.info("Ignoring remaining sequences {} as they do not compose a group of enough orthologs.".format(dRemain.keys()))
				
	logger.info("{:d} duplications detected by Treerecs, extracting {:d} groups of at least {} orthologs.".format(len(dupl), nDuplSign, nbSp))
...
def treeTreatment(data, dAlnTree, nbSp, logger):
	...
	"""
	#try:
	data.filtTree = filterTree(data.tree, data.sptree, data.cor)
	logger.info("Cleaning the tree")

	logger.info("Running Treerecs")
	recTree = runTreerecs(data.filtTree, data.sptree, data.o)
	setattr(data, "recTree", recTree)

	lFastaFile = treeParsing(data.ORFs, data.recTree, nbSp, data.o, logger)
