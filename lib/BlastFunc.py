...
def blast(queryFile, outDir, baseName, db, evalue, percId, cov, apiKey, remote, query):
    ...
    if remote:
        ...
        cmdBlast = 'blastn -task blastn -db {:s} -query {:s} -out {:s} \
            ...
            db, queryFile, blastRes, evalue, percId, cov
        )
...
def treatBlast(data, evalue, percID, mincov, APIKey, remote, entryQuery):
    """
    ...
    @param4 mincov: Blast coverage
    @param5 APIKey: Key for the API of NCBI
    @param6 remote: Boolean (online database or not)
    @param7 entryQuery: Strings which will select the database online
