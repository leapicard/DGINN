...
import logging, os
import ete3
import fastaResFun, analyseFun, treeFun, loadFile
...
def supData(filePath, corFile, dirName):
    ...
    with open(out, "w") as newVer:
        newVer.write(fastaResFun.dict2fasta(newDico))
        newVer.close()
    return out
    ...
    with open(recTree, "r") as tree:
                    ...
                    if len(spGp) > int(nbSp) - 1:
                        ...
                        if not already:
                            ...
                            with open(outFile, "w") as fasta:
                                fasta.write(fastaResFun.dict2fasta(dOrtho2Seq))
                                fasta.close()						
        ...
        if len(lOut) > 0:
            ...
            if len(dRemain.keys()) > int(nbSp) - 1:
                ...
                with open(outFile, "w") as fasta:
                    fasta.write(fastaResFun.dict2fasta(dRemain))
                    fasta.close()
                lOut.append(outFile)
            else:
...
def runTreerecs(pathGtree, pathSptree, cor, o):
  ...
  analyseFun.cmd(val, False)


  return recTree

def treeTreatment(data, dAlnTree, nbSp, phymlOpt, endFile):
    """
    ...
    for aln, tree in dAlnTree.items():
        ...
        if endFile != '' :
            res = shutil.copy(recTree, endFile)
        ...
        if recTree:
            ...
            data["duplication"] = lFastaFile
        
    if len(lFastaFile) > 0:
        for orthoGp in data["duplication"]:
            aln = analyseFun.runPrank(orthoGp, data["o"]) 
            tree = analyseFun.runPhyML(aln, phymlOpt, data["o"])
...
#pb runPrank à régler

def treerecsTreatement(data, treerecs):
    if treerecs:
        data["sptree"], treerecs=treeFun.treeCheck(data["sptree"], data["aln"], treerecs)
    if treerecs:
        outCat, corSG = loadFile.filterData(data["sptree"], data["aln"], data["o"])
        data["seqFile"] = outCat
        data["cor"] = corSG

def treeTreatment2(data, parameters, dAlTree, endFile):
    """
    Procedure which execute all functions for the tree step.
    ...
    dAlnTree2 = {}
    lFastaFile = []
    treerecsTreatement(data, parameters["duplication"])

    for aln in dAlTree:
        filtTree = filterTree(dAlTree[aln], 
                      data["sptree"], 
                      data["cor"])
        logger.info("Cleaning the tree")
    ...
    if len(lFastaFile) > 0:
        for orthoGp in data["duplication"]:
            aln = analyseFun.runPrank(orthoGp, data["o"]) 
            tree = analyseFun.runPhyML(aln, parameters["phymlOpt"], data["o"])
            dAlnTree2[aln] = tree+"_phyml_tree.txt"

    logger.info(str(dAlnTree2))
    if len(dAlnTree2)!=0:
        data["dAlTree"] = dAlnTree2
