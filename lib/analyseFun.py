...
def getORFs(catFile, queryName, geneDir):
    ...
    print("Deleted {} sequences as duplicates".format(n))
    
    outORF = sys.argv[3]
    
    a = fastaResFun.dict2fasta(dId2Longest)

    with open(outORF, "w") as outO:
        outO.write(fastaResFun.dict2fasta(dId2Longest))
        outO.close()

    print("Extracted longest ORFs: {:s}".format(outORF))
    return(outORF)
...
def runPhyML(aln, phymlOpt, geneDir):
    ...
    if phymlOpt != "":
        try:
            ...
            print("PhyML couldn't run with the provided info {}, running with default options.".format(phymlOpt))
...
def phyMLTree(data,phymlOpt):
    ...
    logger = logging.getLogger("main.tree")
    dAlnTree = {}
    print("Running PhyML to produce gene phylogenetic tree")
    
    TreesFile = runPhyML(data["aln"], phymlOpt, data["o"])
    data["tree"] = TreesFile+"_phyml_tree.txt"
    print("Reconstructed tree using PhyML: {:s}".format(data["tree"]))
...
def cutLongBranches(aln, dAlnTree, nbSp, LBOpt, logger):
    ...
    @return dAlnTree: Updated dictionary of alignments and their corresponding trees
    """
    print("Looking for long branches.\n")
    ...
    print("Long branches will be evaluated through the {} method (factor {}) \n".format(LBOpt, factor))
    nbSp = int(nbSp)	
    matches = [leaf for leaf in loadTree.traverse() if leaf.dist>longDist]

    if len(matches) > 0:
        print("{} long branches found, separating alignments.".format(len(matches)))
        ...
        for node in matches:
            ...
            if len(dNewAln) > nbSp - 1:
                with open(newAln, "w") as fasta:
                    fasta.write(fastaResFun.dict2fasta(dNewAln))
                    fasta.close()		  
                dAlnTree[newAln] = ""
            else:
                pass
                print("Sequences {} will not be considered for downstream analyses as they do not compose a large enough group.\n".format(dNewAln.keys()))
        ...
        if len(dID2Seq) > nbSp - 1:
            with open(alnLeft, "w") as fasta:
                fasta.write(fastaResFun.dict2fasta(dID2Seq))
                print("\tNew alignment:%s \n"%{alnLeft})
                fasta.close()	  
            dAlnTree[alnLeft] = ""
            
        else:
            pass
            print("Sequences in {} will not be considered for downstream analyses as they do not compose a large enough group.\n".format(dID2Seq.keys()))
        dAlnTree.pop(aln, None)
        
    else:
        pass
        print("No long branches found.")
    return(dAlnTree)
...
def checkPhyMLTree(data, dAlnTree, nbSp, LBopt, step="option"):
    ...
    for aln in dAlnTree:
        if dAlnTree[aln] == "":
            print("Reconstructing alignments and phylogenies following long branch parsing.\n")
...
def checkPhyMLTree2(data, dAlnTree, nbSp, LBopt, step="option"):
    logger=logging.getLogger(".".join(["main",step]))
...
def runGARD(aln, o, hostFile, logger, endFile):
    """
    ...
    if endFile != '' :
        res = shutil.copy(gardRes, endFile)
...
def parseGard(kh, aln, o, logger):
    ...
    if len(lBP) > 0:
        pass
        print("There are {:d} significant breakpoints in alignement {:s} at positions {}".format(len(lBP), aln, lBP))
    else:
        
        print("There are no significant breakpoints in alignement {:s}.".format(aln))
        return []
    ...
    if len(lBP) > 0:
        ...
        for x in range(1,len(lBP)):
            ...
            with open(outFrag, "w") as outF:
                outF.write(fastaResFun.dict2fasta(dFrag))
                print("\tNew alignment: %s"%{outFrag})
                outF.close()
...
def gardRecomb(data, dAT, hostFile, endFile):
    """
    ...
    dFrag = {}
    for aln in dAT:
        print("Running GARD on {:s}".format(aln))
        gardRes = runGARD(aln, 
                data["o"], 
                hostFile, 
                log, endFile)
            
        print("Checked for recombination using HYPHY GARD.")
                
        parsedFragments = parseGard(gardRes, aln, data["o"], logger)

        print("Parsed GARD results.")
        if len(parsedFragments) > 0:
            for frag in parsedFragments:
                print("Running Phyml on fragment: {:s}".format(frag))
                fragTree = runPhyML(frag, "",  data["o"])
                dFrag[frag] = fragTree+"_phyml_tree.txt"
        else:
            pass
            print("No fragments of recombination identified.")
